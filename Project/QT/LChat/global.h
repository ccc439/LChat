#ifndef GLOBAL_H
#define GLOBAL_H

#include <QWidget>
#include "QStyle"
#include <functional>
#include <QRegularExpression>
#include <QNetworkReply>
#include <QJsonObject>
#include <QDir>
#include <QSettings>


extern std::function<void(QWidget *)> repolish;//用于刷新qss
extern std::function<QString(QString)> xorString;//用于给用户密码加密

//模块功能
enum ReqId{
    ID_GET_VERIFY_CODE = 1001,//获取验证码
    ID_REG_USER = 1002,//注册用户
    ID_RESET_PWD = 1003,//重置密码
    ID_LOGIN_USER = 1004,//用户登录
    ID_CHAT_LOGIN = 1005,//登录聊天服务器
    ID_CHAT_LOGIN_RSP = 1006,//登录聊天服务器回包
    ID_SEARCH_USER_REQ = 1007,//搜索用户
    ID_SEARCH_USER_RSP = 1008,//搜索用户回包
    ID_ADD_FRIEND_REQ = 1009,//申请添加好友
    ID_ADD_FRIEND_RSP = 1010, //申请添加好友回复
    ID_NOTIFY_ADD_FRIEND_REQ = 1011,  //通知用户添加好友申请
    ID_AUTH_FRIEND_REQ = 1012,  //认证好友请求
    ID_AUTH_FRIEND_RSP = 1013,  //认证好友回复
    ID_NOTIFY_AUTH_FRIEND_REQ = 1014, //通知用户认证好友申请
    ID_TEXT_CHAT_MSG_REQ  = 1015,  //文本聊天信息请求
    ID_TEXT_CHAT_MSG_RSP  = 1016,  //文本聊天信息回复
    ID_NOTIFY_TEXT_CHAT_MSG_REQ = 1017, //通知用户文本聊天信息
};

//模块
enum Modules{
    REGISTER_MODULE = 0,//注册模块
    RESET_MODULE = 1,//重置密码模块
    LOGIN_MODULE = 2,//登录模块
};

//错误码
enum ErrorCodes{
    SUCCESS = 0,//成功
    ERR_JSON = 1,//json 解析失败
    ERR_NETWORK = 2,//网络错误

};

//tcp服务器信息
struct ServerInfo{
    QString Host;
    QString Port;
    QString Token;
    int Uid;
};

extern QString gate_url_prefix;//从配置文件中获取完整的HTTP URL

//聊天界面的几种模式
enum ChatUIMode{
    ChatMode,//聊天模式
    ContactMode,//通讯录模式
    SearchMode,//搜索列表模式
};

//自定义QListWidgetItem的几种类型
enum ListItemType{
    CHAT_USER_ITEM,//聊天用户
    CONTACT_USER_ITEM,//联系人用户
    SEARCH_USER_ITEM,//搜索到的用户
    ADD_USER_TIP_ITEM,//提示添加用户
    INVALID_ITEM,//不可点击条目
    GROUP_TIP_ITEM,//分组提示条目
    LINE_ITEM,  //分割线
    APPLY_FRIEND_ITEM, //好友申请
};

//测试
extern std::vector<QString>  strs;

extern std::vector<QString> heads;

extern std::vector<QString> names;

//
enum class ChatRole{
    Self,
    Other
};

struct MsgInfo{
    QString msgFlag;//"text,image,file"
    QString content;//文件和图像的url，文本信息
    QPixmap pixmap;//文件和图像的略缩图
};

//
enum ClickLbState{
    Normal = 0,
    Selected = 1
};
//申请好友标签输入框最低长度
const int MIN_APPLY_LABEL_ED_LEN = 40;

const QString add_prefix = "添加标签 ";

const int  tip_offset = 5;

const int CHAT_COUNT_PER_PAGE = 13;

#endif // GLOBAL_H

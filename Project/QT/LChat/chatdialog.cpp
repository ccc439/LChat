...
ChatDialog::ChatDialog(QWidget *parent)
    ...
    _premode(ChatUIMode::ChatMode),_b_loading(false),_cur_chat_uid(0),_last_widget(nullptr)
{
    ui->setupUi(this);
    ...
    QAction *clearAction = new QAction(ui->search_edit);// 创建一个清除动作并设置图标
    clearAction->setIcon(QIcon(":/res/close_transparent.png"));

    ui->search_edit->addAction(clearAction, QLineEdit::TrailingPosition);// 将清除动作添加到LineEdit的末尾位置
    ...
    connect(clearAction, &QAction::triggered, [this, clearAction]() {
        ...
        //清除按钮被按下则不显示搜索框，显示先前界面
        //若当前界面并非搜索用户界面，则不用处理
        if(_mode == ChatUIMode::SearchMode){
            ShowChatUIMode(_premode);
        }
    });

    ShowChatUIMode(_mode);//默认显示聊天模式
    ...
    //-----对于ChatMode ContactMode SearchMode的处理-----

    ui->side_chat_btn->setChecked(true);//默认选中聊天按钮

    //1.当按下side_chat_btn或ContactMode时，切换至对应界面
    connect(ui->side_chat_btn,&SideChatBtn::sig_press_chat,[=](){
        ShowChatUIMode(ChatUIMode::ChatMode);
    });
    connect(ui->side_contact_btn,&SideContactBtn::sig_press_contact,[=](){
        ShowChatUIMode(ChatUIMode::ContactMode);
    });

    //2.当按下side_chat_btn或ContactMode时，通知另一个按钮
    connect(ui->side_chat_btn,&SideChatBtn::sig_press_chat,ui->side_contact_btn,&SideContactBtn::slot_press_chat);
    connect(ui->side_contact_btn,&SideContactBtn::sig_press_contact,ui->side_chat_btn,&SideChatBtn::slot_press_contact);
    //从联系人列表按下 msg_chat按钮 跳转时也要通知另一个按钮
    connect(this,&ChatDialog::sig_jump_chat,ui->side_contact_btn,&SideContactBtn::slot_press_chat);


    //3.当搜索栏search_edit输入时，切换至搜索列表界面
    connect(ui->search_edit,&QLineEdit::textEdited,[=](const QString &text){
        //若输入被清空，自动退回先前界面
        if (text.isEmpty()){
            ShowChatUIMode(_premode);
        }
        else{
        ShowChatUIMode(ChatUIMode::SearchMode);
        }
    });
    //-----------------------------------------------
...
void ChatDialog::ShowChatUIMode(ChatUIMode mode)
{
    //如果要切换的界面和当前界面一致，不更新_premode
    if(_mode != mode)_premode = _mode;

    if(mode == ChatUIMode::ChatMode){
        ...
        //右侧stackedWidget
        ChatPage *page = ui->chat_page;
        ui->stackedWidget->setCurrentWidget(page);
    }
    else if(mode == ChatUIMode::ContactMode){
        ...
        //右侧stackedWidget
        ApplyFriendPage *page = ui->apply_page;
        ui->stackedWidget->setCurrentWidget(page);
    }
    else if(mode == ChatUIMode::SearchMode){
        ui->chat_user_list->hide();
        ui->con_user_list->hide();
        ui->search_list->show();
...
void ChatDialog::handleGlobalMousePress(QMouseEvent *event)
{
    ...
    QPoint posInSearchList = event->globalPosition().toPoint();
    // 判断点击位置是否在聊天列表的范围内
...
void ChatDialog::slot_apply_friend(std::shared_ptr<AddFriendApply> apply)
{
    ...
    ui->apply_page->AddNewApply(apply);
}
...
void ChatDialog::slot_jump_chat_item_from_infopage(std::shared_ptr<UserInfo> user_info)
{
    ...
    if(find_iter != _chat_items_added.end()){
        ...
        ui->chat_user_list->scrollToItem(find_iter.value());
        ui->side_chat_btn->setChecked(true);

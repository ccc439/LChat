...
TcpMgr::TcpMgr():_host(""),_port(0),_b_recv_pending(false),_message_id(0),_message_len(0)
{
    ...
    QObject::connect(&_socket, &QTcpSocket::readyRead, [&]() {//当有新数据可读时（会粘包），&QTcpSocket::readyRead会被触发，从而调用本lambda
        // 当有数据可读时，读取所有数据
        // 读取所有数据并追加到缓冲区
        ...
        forever {
            ...
            //buffer剩余长度是否满足消息体长度，不满足则退出，继续等待接受
            ...
            // 读取消息体
            QByteArray messageBody = _buffer.mid(0, _message_len);//获取_buffer消息体（mid用于获取子数组）
            qDebug() << "receive body msg is " << messageBody ;

            _buffer = _buffer.mid(_message_len);//保留剩余数据
...
void TcpMgr::initHandlers()
{
    ...
    _handlers.insert(ID_CHAT_LOGIN_RSP, [this](ReqId id, int len, QByteArray data){
        ...
        if(jsonObj.contains("token")){
            ...
            qDebug() << "Token set successfully";
        } else {
            qDebug() << "WARNING: No token field in login response!";
            UserMgr::getInstance()->SetToken("");
        }
        
        // 存储好友申请信息到_apply_list
        if(jsonObj.contains("apply_list")){
            qDebug() << "Found apply_list, size:" << jsonObj["apply_list"].toArray().size();
            UserMgr::getInstance()->AppendApplyList(jsonObj["apply_list"].toArray());
        } else {
            qDebug() << "No apply_list field in response";
        }

        // 存储好友信息到friend_list、_friend_map
        if (jsonObj.contains("friend_list")) {
            qDebug() << "Found friend_list, size:" << jsonObj["friend_list"].toArray().size();

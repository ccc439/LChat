...

{
    ...
    // 连接好友列表更新信号，在登录成功后刷新联系人列表
    connect(TcpMgr::getInstance().get(), &TcpMgr::sig_friend_list_updated, this,
            &ContactUserList::slot_refresh_contact_list);
}
...
void ContactUserList::initFixedItems()
{
    qDebug() << "initFixedItems called - initializing fixed UI items";
    ...
    _add_friend_item->SetInfo(0,tr("新的朋友"),":/res/add_friend_hover.png");
    ...
    qDebug() << "initFixedItems finished - fixed items initialized";
}

void ContactUserList::loadContactList()
{
    qDebug() << "loadContactList called - loading real contact data from UserMgr";
    
    //加载后端发送过来的好友列表
    auto con_list = UserMgr::getInstance()->GetConListPerPage();
    qDebug() << "GetConListPerPage returned, list size:" << con_list.size();
    
    for(auto & con_ele : con_list){
        auto *con_user_wid = new ConUserItem();
        con_user_wid->SetInfo(con_ele->_uid,con_ele->_name, con_ele->_icon);
        qDebug()<<"Loading contact user: uid=" << con_ele->_uid 
                << ", name=" << con_ele->_name 
                << ", icon=" << con_ele->_icon;
    ...
    qDebug() << "loadContactList finished - loaded" << con_list.size() << "contacts";
}
...
void ContactUserList::slot_refresh_contact_list()
{
    qDebug() << "slot_refresh_contact_list called! Refreshing contact list...";
    
    // 清除现有的联系人项（保留"新的朋友"和"联系人"分组）
    // 从后往前删除，避免索引变化
    for (int i = this->count() - 1; i >= 0; --i) {
        ...
        // 跳过"新的朋友"和"联系人"分组项
        if (widget == _add_friend_item || item == _groupitem) {
            continue;
        }
    ...
    // 重新加载联系人列表（使用新的 loadContactList 函数）
    loadContactList();
    
    qDebug() << "slot_refresh_contact_list finished!";
}
